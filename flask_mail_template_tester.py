...
@click.option('-d', '--data', 'dataFile', help="data file must set a template_data object to be given to the template. $TEMPLATE_DATA_FILE or cwd()/template_test_data.py")
...
def test_template(templatename, dataFile, email):
    ...
    dataFile = dataFile or os.getenv('TEMPLATE_DATA_FILE') or os.path.join(os.getcwd(), 'template_test_data.py')
